...
import { createIncomeForecast, editIncomeForecast, fetchSingleIncomeForecast, updateIncomeForecastForm } from '../../actions/income-forecast-action-types';
import { fetchSchoolYear } from '../../actions/school-year-action-type';
import { Button } from '../../components';
import Regex from '../../utility/regex';
import Alert from '../../utility/alert';

export const AddEditIncomeForecast = () => {
  ...
  const {
    ...
    form: store.incomeForecast.form,
  ...
  React.useEffect(() => {
    ...
    if (params && params.publicId) {
      dispatch(fetchSingleIncomeForecast(params.publicId));
    }
  }, []);
  ...
  const onChange = (e) => {
    ...
    dispatch(updateIncomeForecastForm(updates));
  };
  ...
  const onSave = () => {
    ...
    if (params.publicId) {
      ...
      dispatch(editIncomeForecast(request));
    } else {
      dispatch(createIncomeForecast(form));
    }
  };

  const onNavigate = () => dispatch(push('/income-forecast'));

  return (
    <div className="mb-3 card">
      <div className="card-header-tab card-header">
        <div className="card-header-title font-size-lg text-capitalize font-weight-normal flex-fill">
          {disabled ? 'View Income Forecast' : `${params.publicId ? 'Edit Income Forecast' : 'Add New Income Forecast'}`}
        </div>
        {disabled && (
...
export default AddEditIncomeForecast;
